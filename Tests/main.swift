...
let loop = [InkPoint(x: 50, y: 50), InkPoint(x: 350, y: 50), InkPoint(x: 350, y: 300), InkPoint(x: 50, y: 300)]
let insideStroke = InkStroke(points: [InkPoint(x: 80, y: 80, pressure: 0.3), InkPoint(x: 150, y: 110, pressure: 0.8)])
let crossingStroke = InkStroke(points: [InkPoint(x: 10, y: 200), InkPoint(x: 400, y: 200)])
let outsideStroke = InkStroke(points: [InkPoint(x: 500, y: 500), InkPoint(x: 600, y: 550)])
let insideText = TextCard(text: "Move together", x: 70, y: 120)
let outsideText = TextCard(text: "Leave here", x: 400, y: 600)
let selectionPage = NotePage(strokes: [insideStroke, crossingStroke, outsideStroke], texts: [insideText, outsideText])
let group = LassoRegion(points: loop).selection(in: selectionPage)
check(group.strokes == [insideStroke.id, crossingStroke.id] && group.texts == [insideText.id], "Lasso selects enclosed ink and text, including segments crossing between samples")
check(LassoRegion(points: Array(loop.reversed())).selection(in: selectionPage) == group, "Lasso works in either drawing direction")
check(LassoRegion(points: [loop[0], loop[1]]).selection(in: selectionPage).count == 0, "An unclosed straight drag does not select a group")
let concave = LassoRegion(points: [InkPoint(x: 0, y: 0), InkPoint(x: 200, y: 0), InkPoint(x: 200, y: 50), InkPoint(x: 50, y: 50), InkPoint(x: 50, y: 200), InkPoint(x: 0, y: 200)])
check(concave.contains(InkPoint(x: 25, y: 100)) && !concave.contains(InkPoint(x: 100, y: 100)), "Concave lasso follows the drawn shape rather than its bounding box")
check(LassoRegion(points: loop).contains(InkPoint(x: 50, y: 100)), "Lasso includes objects exactly on its boundary")
let tinyLoop = [InkPoint(x: 90, y: 140), InkPoint(x: 110, y: 140), InkPoint(x: 110, y: 160), InkPoint(x: 90, y: 160)]
check(LassoRegion(points: tinyLoop).selection(in: selectionPage).texts == [insideText.id], "Circling within a text area selects that text")
let movedGroup = group.moving(selectionPage, by: CGSize(width: 40, height: 60))
check(movedGroup.strokes[0].points[0].x == 120 && movedGroup.strokes[1].points[0].x == 50 && movedGroup.texts[0].x == 110, "Group movement preserves spacing between multiple strokes and text")
check(movedGroup.strokes[2] == outsideStroke && movedGroup.texts[1] == outsideText, "Group movement leaves unselected objects untouched")
check(movedGroup.strokes[0].id == insideStroke.id && movedGroup.strokes[0].points[0].pressure == 0.3 && selectionPage.strokes[0] == insideStroke, "Group movement preserves identities, pressure, and the undo snapshot")
let edgeGroup = group.moving(selectionPage, by: CGSize(width: 10000, height: 10000))
let edgeBounds = group.bounds(in: edgeGroup)!
