...
- **Export (share icon):** choose editable `.noted`, PDF, PNG, or JPEG, then All pages, Page range, or Single page. Integer sliders select pages, with starting/ending previews. **Share** opens the native iPad share sheet or Mac sharing picker; available destinations depend on the device. iPad's share sheet includes Save to Files when available, and Mac also has a separate **Save a copy** button. PDF/image exports can omit ruled/grid paper. Image exports produce one file per selected page at 1536 × 2048. Exporting does not move the working notebook.
...
- **Selection:** tap again to choose Freehand or Rectangle. Select ink, text and placed images, then drag inside the box to move. Drag the lower-right circular handle to resize; its touch target stays 44 points wide at every zoom level and extends beyond the selection boundary. Horizontal, vertical and diagonal pulls all resize groups/images proportionally; resizing a single text item changes its wrapping width without stretching the font. A nearby menu provides Copy, Duplicate, Delete and Deselect. Use Insert → Paste selection to paste editable content copied in Noted, including into another notebook; an image on the system clipboard can also be pasted.
...
- **Navigation:** two fingers pan or pinch on iPad, from 40% to 400% of the fitted page size. Pinch no longer stops at 100%, reconfigures the pan recognizer on each frame, or starts a fling when lifting the fingers. Panning has cancellable momentum, which cannot create extra pages. Touching the canvas or switching tools stops momentum. Mac trackpad scrolling/pinching remains supported; Hand enables mouse dragging. Tap the zoom percentage to fit the current page. iPad starts with its page sidebar hidden and a compact title; the sidebar control reveals it.
...
Sharing prepares temporary, independent file snapshots. They are removed after sharing is dismissed or completed; snapshots left by an interrupted app session are cleaned up on a later export after seven days. Cancelling sharing leaves the working notebook untouched.

## Pages and templates

New notebooks start with **ten empty pages** using the chosen template. Existing notebooks and imports retain their original page count. Pages sit in a vertical stack with a small 12-point gap. Use **Insert → Blank page** or the sidebar **+** to choose a previewed **Ruled**, **Grid**, or **Blank** template for another page. Notebook menu → Page templates changes the current page's paper without moving or flattening its contents. Page duplication, reordering, and deletion remain in the notebook menu.

**Add pages as you scroll** is enabled by default and can be disabled in the notebook menu or the template chooser. Scroll through existing pages; deliberately scrolling beyond the final page adds another empty page with the same paper, including when the last page is blank. Writing or typing does not create pages. A small overscroll threshold prevents tiny edge movements from immediately creating pages. Scroll upward to return to earlier pages, or choose a page in the sidebar. Zooming alone never adds pages. Page creation can be undone separately from edits. Visible pages and nearby pages are kept mounted so they are ready before entering the viewport. Very large notebooks still require performance testing, and reduced stutter needs physical-device confirmation.
...
- **89 model checks:** existing ink/text/undo-related model behavior, viewport and lasso geometry, automatic-page behavior, ten-page creation without changing existing files, zooming below fit on later pages, stack bounds and gaps, version 1 compatibility, version 2 media round trips, malformed references, page ranges, subset asset selection, repeated imports, exact partial erasing, version 3 compatibility, image/text selection, clamped movement, axis-aligned/proportional resizing, resized-content round trips, and invalid modern geometry.
...
- September 21 feedback fixes add ten-page creation, neighboring-page preloading, pinch fixes, a direct resize handle, slider-based export and native sharing. Automated checks and both builds pass; actual scroll smoothness, two-finger gestures, Pencil resizing and AirDrop/app sharing need the user's next manual checkpoint.
